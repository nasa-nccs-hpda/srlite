...
def processToa(toa, contextClazz, context, rasterLib):
    ...
    try:
        if (num_workers > 0):
            ...
            for i in range(num_workers):
                print(f'Starting ProcessingPool().amap() for toa: {str(toaList[i])}', flush=True)
                result = amap(processToa, [toaList[i]], [contextClazz], [context], [rasterLib])
                results.append(result)

            for j in range(num_workers):
                result = results[j].get()
                print(f'Ending ProcessingPool().amap() for toa: {str(toaList[j])} {result}', flush=True)

            # result2 = amap(processToa, [toaList[1]], [contextClazz], [context], [rasterLib])
 
            # print(f'Starting ProcessingPool().amap() for toas: {str(toaList)}', flush=True)
            # for results in results.get():
            #     print(f'Got result: {results}', flush=True)
            # # for result1 in result1.get():
            #     print(f'Got result: {result1}', flush=True)

            # with Pool(num_workers) as pool:
            #     # issue tasks to process pool
            #     result = pool.starmap_async(processToa, items)
            #     # iterate results
...
def _processToa(toa, contextClazz, context, rasterLib):
